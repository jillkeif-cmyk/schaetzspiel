...
app.post('/api/news/read', async (req, res) => {
  try {
    const u = await auth(req); if (!u) return res.status(401).json({ error: 'Bitte neu anmelden.' });
    const latest = NEWS.find(newsVisible); const id = String(req.body.id || ''); // nur sichtbare Beiträge
...
app.get('/api/news', (req, res) => res.json({ posts: NEWS.filter(newsVisible) }));
...
app.post('/api/admin/openstyle', async (req, res) => {
  const u = await auth(req); if (!u || !isAdmin(u)) return res.status(403).json({ error: 'Nur für den Admin.' });
  openStyleV = req.body.style === 'klassisch' ? 'klassisch' : 'showroom'; await store.setting('open_style', openStyleV); io.emit('openstyle', openStyleV); res.json({ style: openStyleV });
});
...
async function tripleFinish(uid, st, extraPay) { // Gewinn auszahlen und Runde für Casino-XP verbuchen
  ...
  let dia = null;
  const gname = GAME_LABEL[st.game] || 'Triple Crown';
...
app.post('/api/admin/display', async (req, res) => {
  const u = await auth(req); if (!u || !isAdmin(u)) return res.status(403).json({ error: 'Nur für den Admin.' });
  const p = tcg.PACKS[String(req.body.pack || '')]; if (!p || !(p.display || p.potion)) return res.status(400).json({ error: 'Unbekannter Artikel.' });
...
app.post('/api/tcg/buy', async (req, res) => {
  try {
    ...
    if (p.hidden && !p.display && !p.potion) return res.status(403).json({ error: 'Diesen Booster gibt es nicht im Shop.' });
    if (p.potion && (p.off || !(p.price > 0)) && !isMod(u)) return res.status(403).json({ error: 'Diesen Trank gibt es gerade nicht im Shop.' });
    if (p.display && p.off && !isMod(u)) return res.status(403).json({ error: 'Das Display gibt es gerade nicht zu kaufen.' });
    if (p.offsale && !isMod(u)) return res.status(403).json({ error: 'Diesen Booster gibt es gerade nicht im Shop.' });
    if ((p.locked || (p.display && tcg.PACKS[p.of] && tcg.PACKS[p.of].locked)) && !isMod(u)) return res.status(403).json({ error: 'Dieser Booster ist noch gesperrt. Bald geht es los!' });
    if (p.soldout) return res.status(403).json({ error: p.potion ? 'Dieser Trank ist ausverkauft.' : p.display ? 'Dieses Display ist ausverkauft.' : 'Dieser Booster ist ausverkauft.' });
...
app.post('/api/tcg/open', async (req, res) => {
  try {
    ...
    if (P.display) { // Display öffnen: alle Booster wandern ins Inventar
      await store.packAdd(u.id, pid, -1); await store.packAdd(u.id, P.of, P.size);
      console.log(`Display geöffnet: ${u.name} ${P.name} -> ${P.size} Booster`);
      return res.json({ display: true, added: P.size, of: P.of, state: await tcgState(await store.userById(u.id)) });
    }
...
app.post('/api/tcg/meltdisplay', async (req, res) => {
  try {
    ...
    if (p.of === 'gn' && !gnOpen) return res.status(403).json({ error: 'Gruselnacht-Displays lassen sich erst nach der Freischaltung umwandeln.' });
    const have = ((await store.packsOf(u.id)).find((x) => x.pack_id === p.id) || {}).count || 0, n = Math.min(have, Math.max(1, Math.round(Number(req.body.n) || 1)));
    if (!have) return res.status(400).json({ error: 'Du hast dieses Display nicht.' });
    const gain = dispMelt(p) * n, fresh = await store.userById(u.id);
    await store.packAdd(u.id, p.id, -n); await store.save(u.id, { diamonds: (Number(fresh.diamonds) || 0) + gain });
    res.json({ ...(await tcgState(await store.userById(u.id))), gained: gain, n });
  } catch (e) { console.error(e); res.status(500).json({ error: 'Serverfehler.' }); }
});
...
// Code einlösen
let homeCfg = { newsCard: true, promo: { on: false, game: 'crypt', text: '' } }; // Startseite: Update-Karte und Automaten-Werbung (Admin-Menü)
store.setting('home_cfg').then((v) => { if (v) { try { const c = JSON.parse(v); homeCfg = { ...homeCfg, ...c, promo: { ...homeCfg.promo, ...(c.promo || {}) } }; } catch (e) {} } }).catch(() => {});
...
app.post('/api/admin/home', async (req, res) => { // Startseite: Update-Karte an/aus, Automaten-Werbung (Spiel, Text, an/aus)
  const u = await auth(req); if (!u || !isAdmin(u)) return res.status(403).json({ error: 'Nur für den Admin.' });
  const b = req.body || {};
  if ('newsCard' in b) homeCfg.newsCard = !!b.newsCard;
  ...
  await store.setting('home_cfg', JSON.stringify(homeCfg)); io.emit('home:cfg', { newsCardOff: !homeCfg.newsCard, promo: homeCfg.promo.on ? { game: homeCfg.promo.game, text: homeCfg.promo.text } : null }); res.json({ homeCfg });
});
app.post('/api/admin/tabbadge', async (req, res) => { // Abzeichen setzen oder entfernen (leerer Text = weg)
  const u = await auth(req); if (!u || !isAdmin(u)) return res.status(403).json({ error: 'Nur für den Admin.' });
  const key = String(req.body.key || '').replace(/[^a-z]/g, '').slice(0, 20), text = String(req.body.text || '').trim().slice(0, 12);
  if (!key) return res.status(400).json({ error: 'Unbekannter Reiter.' });
  if (text) tabBadges[key] = text; else delete tabBadges[key];
  await store.setting('tab_badges', JSON.stringify(tabBadges)); io.emit('shop:changed'); res.json({ badges: tabBadges });
});
app.post('/api/admin/slotsrelease', async (req, res) => { // Ein Knopf: beide Automaten für alle, Update 14 sichtbar, Code AUTOMATEN
  const u = await auth(req); if (!u || !isAdmin(u)) return res.status(403).json({ error: 'Nur für den Admin.' });
  jesterTest = false; anubisTest = false; slotsLive = true;
  await store.setting('jester_test', '0'); await store.setting('anubis_test', '0'); await store.setting('slots_live', '1');
...
app.post('/api/admin/anubistest', async (req, res) => { // Auge des Anubis: Testphase an/aus
  const u = await auth(req); if (!u || !isAdmin(u)) return res.status(403).json({ error: 'Nur für den Admin.' });
  anubisTest = !!req.body.on; await store.setting('anubis_test', anubisTest ? '1' : '0'); io.emit('shop:changed');
  console.log(`Auge des Anubis Testphase ${anubisTest ? 'an' : 'aus'} durch ${u.name}`); res.json({ on: anubisTest });
});
app.post('/api/casino/anubis', async (req, res) => { // Auge des Anubis: Maschinen 5 und 6, Freispiele werden am Stück berechnet
  try {
    if (gameLocked('anubis', res)) return;
    const u = await auth(req); if (!u) return res.status(401).json({ error: 'Bitte neu anmelden.' });
    if (anubisBlocked(u, res)) return;
    const bet = Math.round(Number(req.body.bet) || 0);
    if (!anubis.BETS.includes(bet)) return res.status(400).json({ error: 'Ungültiger Einsatz.' });
    if (!tcSeated(u, res, req.body.machine)) return;
    if (machineGame(tcSeatOf(u.id)) !== 'anubis') return res.status(400).json({ error: 'Setz dich an eine Maschine von Auge des Anubis.' });
    if (!tripleLock(u.id, res)) return;
    try {
      const open = tripleOpen.get(u.id); if (open) await tripleFinish(u.id, open, open.win);
      const u2 = await store.userById(u.id);
...
app.post('/api/admin/bigwin', async (req, res) => { // Schwelle für die „großer Gewinn“-Meldung an alle
  const u = await auth(req); if (!u || !isAdmin(u)) return res.status(403).json({ error: 'Nur für den Admin.' });
  BIGWIN_MIN = Math.max(0, Math.round(Number(req.body.min) || 0)); await store.setting('bigwin_min', String(BIGWIN_MIN));
  console.log(`Gewinn-Meldung ab ${BIGWIN_MIN} durch ${u.name}`); res.json({ min: BIGWIN_MIN });
});
...
app.get('/api/invitable', async (req, res) => { // alle Spieler zum Einladen, online zuerst (nicht nur Freunde)
  try {
    const u = await auth(req); if (!u) return res.status(401).json({ error: 'Bitte neu anmelden.' });
    const all = (await store.usersFull(400)).filter((o) => o.id !== u.id);
...
function tcAttach(socket, user) { // Triple-Crown-Maschinen: Liste, Zuschauen, automatisch aufstehen
  ...
  socket.on('tc:watch', (id) => {
    ...
    if (st) notifyWatched(st.uid, user, GAME_LABEL[machineGame(m)] || 'Triple Crown');
  ...
  socket.on('cw:watch', (id) => {
    ...
    if (!socket.data.cwWatching || socket.data.cwWatching !== x[0]) { socket.data.cwWatching = x[0]; notifyWatched(x[0], user, x[1].game === 'bj' ? 'Blackjack' : 'Roulette'); }
