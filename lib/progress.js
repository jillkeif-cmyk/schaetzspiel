...
const LABEL = { wins: 'Siege', matches: 'Matches', exact: 'Punktlandungen', close: 'Doppelte (×2)', mc_right: 'Auswahlfragen richtig', answered: 'Fragen beantwortet', best_score: 'Punkte in einem Match', best_streak: 'Siege in Folge' };
const PRESTIGE_NAMES = ['', 'Bronzewinkel', 'Messschieber', 'Volltreffer', 'Silbereule', 'Goldkompass', 'Goldwolf', 'Smaragdkobra', 'Sturmadler', 'Rubinkrone', 'Flamme'];
...
const RANK_BASE = [30, 18, 10, 6, 3, 1];
function rankPointsFor(place, players, exact) {
  const base = RANK_BASE[Math.min(place, 5)] || 1;
  return Math.round(base * (0.7 + 0.1 * Math.min(players, 6))) + 5 * (exact || 0);
}
...
function applyMatch(u, r) {
  ...
  if (r.exact) parts.push([`Punktlandung ×${r.exact}`, 500 * r.exact]);
  const unlocked = [];
...
module.exports = { rankPointsFor, MAX_LEVEL, MAX_PRESTIGE, CAP, PRESTIGE_NAMES, levelInfo, prestigeStatus, challengeView, applyMatch };
