...
const CHALLENGES = [
  { key: 'toon_distinct', name: 'Toon-Sammler', desc: 'Sammle verschiedene Toon-Welt-Karten', tiers: [3, 8, 15, 22, 27, 30], xp: [200, 400, 800, 1600, 3200, 6400], group: 'toon', isNew: true },
...
const LABEL = { toon_distinct: 'Toon-Welt-Karten', casino_rounds: 'Casino-Runden', casino_wins: 'Casino-Siege', casino_best: 'Bester Casino-Gewinn', cards_total: 'Karten', cards_rare: 'Seltene Karten', cards_ext: 'Extended-Art-Karten', points: 'Punkte insgesamt', rank_points: 'Weltranglistenpunkte', wins: 'Siege', matches: 'Matches', exact: 'Punktlandungen', close: 'Doppelte (×2)', mc_right: 'Auswahlfragen richtig', answered: 'Fragen beantwortet', best_score: 'Punkte in einem Match', best_streak: 'Siege in Folge' };
...
function challengeView(u) {
  return CHALLENGES.map((c) => {
    ...
    return { key: c.key, name: c.name, desc: c.desc, cur, done, total: c.tiers.length, goal: c.tiers[done] ?? null, reward: c.xp[done] ?? null, group: c.group || (c.key.startsWith('casino_') ? 'casino' : c.key.startsWith('cards_') ? 'cards' : 'quiz'), isNew: !!c.isNew };
  });
}
