...
// XP von Level lvl auf lvl+1. Bis Prestige 4 wie bisher (flach, schnell). Ab Prestige 5 steigend: anfangs schnell,
// zum Ende deutlich langsamer (+12 % je Level), dazu +20 % je Prestige über 5.
const NEW_CURVE_FROM = 5;
const need = (lvl, p = 0) => (p >= NEW_CURVE_FROM ? Math.round(400 * Math.pow(1.12, lvl - 1) * (1 + 0.2 * (p - NEW_CURVE_FROM)) / 10) * 10 : 150 + 25 * lvl);
const capFor = (p = 0) => Array.from({ length: MAX_LEVEL - 1 }, (_, i) => need(i + 1, p)).reduce((a, b) => a + b, 0);
const CAP = capFor(0); // alte Obergrenze (Prestige 0–4)

function levelInfo(xp, p = 0) {
  const cap = capFor(Number(p) || 0); let lvl = 1, rest = Math.min(Number(xp) || 0, cap);
  while (lvl < MAX_LEVEL && rest >= need(lvl, p)) { rest -= need(lvl, p); lvl++; }
  return { level: lvl, into: lvl === MAX_LEVEL ? 0 : rest, next: lvl === MAX_LEVEL ? 0 : need(lvl, p), max: lvl === MAX_LEVEL, xpTotal: Math.max(0, Math.min(Number(xp) || 0, cap)), xpCap: cap };
}
...
const CHALLENGES = [
  ...
  { key: 'collect_unique', name: 'Komplettist', desc: 'Sammle verschiedene Karten, jede Fassung zählt einmal', tiers: [25, 75, 150, 250, 295, 440], xp: [400, 1000, 2500, 6000, 15000, 20000], group: 'cards' }, // 440 = alle Karten inkl. Gruselnacht
...
// Prestige-Bedingungen zählen nur, was seit Erreichen der aktuellen Stufe dazukam (pres_base = Stand beim Aufstieg).
// Bestwerte zählen innerhalb der Stufe (p_best_score, p_best_streak).
const PRES_SUM = ['wins', 'close', 'exact', 'matches', 'mc_right', 'answered', 'rank_points', 'points'];
const presSnapshot = (u) => JSON.stringify(Object.fromEntries(PRES_SUM.map((k) => [k, Number(u[k]) || 0])));
function presVal(u, k) {
  if (k === 'best_score') return Number(u.p_best_score) || 0;
  if (k === 'best_streak') return Number(u.p_best_streak) || 0;
  let base = {}; try { base = JSON.parse(u.pres_base || '{}') || {}; } catch (e) {}
  return Math.max(0, (Number(u[k]) || 0) - (Number(base[k]) || 0));
}
function prestigeStatus(u) {
  const next = u.prestige + 1;
  if (next > MAX_PRESTIGE) return { next: null, reqs: [], can: false };
  const lv = levelInfo(u.xp, u.prestige);
  const reqs = [{ label: `Level ${MAX_LEVEL}`, cur: lv.level, goal: MAX_LEVEL, done: lv.max },
    ...Object.entries(PRESTIGE_REQ[next]).map(([k, g]) => ({ key: k, label: LABEL[k], cur: Math.min(presVal(u, k), g), goal: g, done: presVal(u, k) >= g }))];
...
function applyMatch(u, r) {
  const before = { ...u }, lvBefore = levelInfo(u.xp, u.prestige);
  const n = { ...u };
  ...
  if (r.counted) { n.streak = r.win ? (u.streak || 0) + 1 : 0; n.best_streak = Math.max(u.best_streak || 0, n.streak); n.p_best_streak = Math.max(Number(u.p_best_streak) || 0, n.streak); }
  n.p_best_score = Math.max(Number(u.p_best_score) || 0, r.score); // Bestwert innerhalb der aktuellen Prestige-Stufe
  ...
  n.xp = Math.min(capFor(u.prestige), u.xp + gained);
  const lvAfter = levelInfo(n.xp, u.prestige);
...
const xpForLevel = (lvl, p = 0) => { let x = 0; for (let i = 1; i < Math.min(Math.max(1, lvl), MAX_LEVEL); i++) x += need(i, p); return x; };

module.exports = { presSnapshot, presVal, capFor, need, NEW_CURVE_FROM, xpForLevel, rankPointsFor, MAX_LEVEL, MAX_PRESTIGE, CAP, PRESTIGE_NAMES, levelInfo, prestigeStatus, challengeView, applyMatch };
