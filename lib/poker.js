...
module.exports = function setupPoker(io, store, onStat, canPlay) {
  ...
  function award(t, pots, byFold) {
    ...
    for (const p of h.players) onStat(p.id, p.contrib, won.get(p.id) || 0).catch(() => {});
    pushTable(t);
    t.timer = setTimeout(() => {
      t.hand = null;
  ...
  return { attach, tables, isSeated: (id) => seatOf.has(id), best, rank5, handName };
};
