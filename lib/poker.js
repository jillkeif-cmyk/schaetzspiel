...
const TURN_MS = 20000, NEXT_HAND_MS = 5000, BETWEEN_MS = 1500, SHOW_MS = 6000;
...
module.exports = function setupPoker(io, store, onStat, canPlay, push) {
  ...
  const summary = () => [...tables.values()].map((t) => ({ id: t.id, name: t.name, buyIn: t.buyIn, sb: t.sb, bb: t.bb, max: t.max, seated: t.seats.filter((s) => s && !s.bot).length, bot: t.seats.some((s) => s && s.bot), running: !!t.hand }));
  const pushList = () => io.emit('pk:list', summary());

  function view(t, uid) {
    const h = t.hand;
    return {
      id: t.id, name: t.name, buyIn: t.buyIn, sb: t.sb, bb: t.bb, max: t.max, dealer: t.dealer, handNo: t.handNo || 0, nextIn: !h && t.nextAt ? Math.max(0, t.nextAt - Date.now()) : 0,
