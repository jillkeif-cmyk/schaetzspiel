...
const NUM_KEYS = ['casino_ban', 'wheel_bonus', 'pass_xp', 'pass_prem', 'pass_bank', 'matches', 'wins', 'answered', 'exact', 'close', 'mc_right', 'mc_total', 'points', 'dev_sum', 'dev_n', 'xp', 'prestige', 'best_score', 'streak', 'best_streak', 'av', 'rank_points', 'pres_shown', 'last_seen', 'diamonds', 'casino_xp', 'casino_rounds', 'casino_wins', 'casino_best', 'casino_net', 'daily_streak', 'daily_base', 'packs_opened', 'melted', 'casino_day_net', 'cash_prev_net', 'wheel_used', 'poker_hands', 'poker_wins', 'poker_best', 'poker_allin_wins', 'play_minutes', 'poker_minutes', 'peak_cards_total', 'peak_cards_rare', 'peak_cards_ext', 'peak_toon', 'peak_collect', 'slot_spins', 'slot_full', 'slot_best'];
const TEXT_KEYS = ['pass_season', 'pass_cf', 'pass_cp', 'pass_wk', 'pass_wdone', 'tag', 'tag_color', 'emblem', 'title', 'codes', 'role', 'unlocks', 'frame', 'seen_items', 'daily_day', 'casino_day', 'cash_prev_day', 'cash_claimed', 'wheel_day', 'casino_theme', 'first_game_day', 'news_claimed', 'showcase', 'showcase_g', 'chal_paid'];
...
function memoryStore() {
  ...
  return {
    ...
    async save(id, f) {
      ...
      const before = Number(u.diamonds) || 0;
      for (const k of [...NUM_KEYS, ...TEXT_KEYS]) if (k in f) u[k] = f[k];
      if ('diamonds' in f && Number(f.diamonds) !== before) ledger.push({ user_id: id, user_name: u.name, ts: Date.now(), delta: Number(f.diamonds) - before, after: Number(f.diamonds), src: src() });
    },
    async ledgerOf(uid, limit = 300) { return ledger.filter((x) => x.user_id === uid).slice(-limit).reverse(); },
...
function pgStore(url) {
  ...
  return {
    ...
    async save(id, f) {
      ...
      if (!('diamonds' in f)) { await db.query(`UPDATE users SET ${keys.map((k, i) => `${k}=$${i + 2}`).join(',')} WHERE id=$1`, [id, ...keys.map((k) => f[k])]); return; }
      // Guthaben-Protokoll: alten Stand im selben Schritt lesen, Differenz festhalten
      const r = await db.query(`WITH old AS (SELECT diamonds FROM users WHERE id=$1) UPDATE users SET ${keys.map((k, i) => `${k}=$${i + 2}`).join(',')} WHERE id=$1 RETURNING (SELECT diamonds FROM old) AS before`, [id, ...keys.map((k) => f[k])]);
      const before = r.rows[0] ? Number(r.rows[0].before) || 0 : 0, after = Number(f.diamonds) || 0;
      if (after !== before) db.query('INSERT INTO ledger(user_id,ts,delta,after,src) VALUES($1,$2,$3,$4,$5)', [id, Date.now(), after - before, after, src()]).catch((e) => console.error('Protokoll:', e.message));
    },
    async ledgerOf(uid, limit = 300) { return (await db.query('SELECT user_id,ts,delta,after,src FROM ledger WHERE user_id=$1 ORDER BY ts DESC, id DESC LIMIT $2', [uid, limit])).rows.map((x) => ({ ...x, ts: Number(x.ts), delta: Number(x.delta), after: Number(x.after) })); },
    async friendAdd(a, b) {
    ...
    async giftAdd(g) { return one('INSERT INTO gifts(user_id,diamonds,pack,n,reason,source,message) VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING *', [g.user_id, g.diamonds || 0, g.pack || null, g.n || 0, g.reason || '', g.source || 'support', g.message || null]); },
