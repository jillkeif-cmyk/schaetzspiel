...
const TEXT_KEYS = ['pot_active', 'pass_season', 'pass_cf', 'pass_cp', 'pass_wk', 'pass_wdone', 'tag', 'tag_color', 'emblem', 'title', 'codes', 'role', 'unlocks', 'frame', 'seen_items', 'daily_day', 'casino_day', 'cash_prev_day', 'cash_claimed', 'wheel_day', 'casino_theme', 'first_game_day', 'news_claimed', 'showcase', 'showcase_g', 'chal_paid'];
...
function memoryStore() {
  ...
  return {
    ...
    async save(id, f) {
      ...
      for (const k of TRACK) if (k in f && Number(f[k]) !== before[k]) ledger.push({ user_id: id, ts: Date.now(), delta: Number(f[k]) - before[k], after: Number(f[k]), src: src(), kind: KIND[k] });
    },
...
function pgStore(url) {
  ...
  return {
    ...
    async save(id, f) {
      ...
      const row = r.rows[0] || {};
      for (const k of tr) { const before = Number(row['o_' + k]) || 0, after = Number(f[k]) || 0; if (after !== before) db.query('INSERT INTO ledger(user_id,ts,delta,after,src,kind) VALUES($1,$2,$3,$4,$5,$6)', [id, Date.now(), after - before, after, src(), KIND[k]]).catch((e) => console.error('Protokoll:', e.message)); }
    },
    async ledgerOf(uid, limit = 300, kind = 'dia') { return (await db.query("SELECT user_id,ts,delta,after,src FROM ledger WHERE user_id=$1 AND COALESCE(kind,'dia')=$3 ORDER BY ts DESC, id DESC LIMIT $2", [uid, limit, kind])).rows.map((x) => ({ ...x, ts: Number(x.ts), delta: Number(x.delta), after: Number(x.after) })); },
    async friendAdd(a, b) {
