...
const NUM_KEYS = ['matches', 'wins', 'answered', 'exact', 'close', 'mc_right', 'mc_total', 'points', 'dev_sum', 'dev_n', 'xp', 'prestige', 'best_score', 'streak', 'best_streak', 'av', 'rank_points'];
const TEXT_KEYS = ['tag', 'tag_color', 'emblem', 'title', 'codes'];
...
function memoryStore() {
  ...
  return {
    ...
    async createUser(name, pass) {
      ...
      const u = { id: ++uid, name, pass, avatar: null, tag: '', tag_color: '', emblem: '', title: '', codes: '', ...Object.fromEntries(NUM_KEYS.map((k) => [k, 0])) };
