...
function memoryStore() {
  ...
  return {
    ...
    async usersByIds(ids) { return ids.map((id) => byId.get(id)).filter(Boolean); },
    async allCards() { return [...inv.values()].filter((x) => x.count > 0); },
    async addMinutes(ids, pokerIds) { for (const id of ids) { const u = byId.get(id); if (u) { u.play_minutes = (Number(u.play_minutes) || 0) + 1; if (pokerIds.includes(id)) u.poker_minutes = (Number(u.poker_minutes) || 0) + 1; } } },
...
function pgStore(url) {
  ...
  return {
    ...
    async allCards() { return (await db.query('SELECT user_id, card_id, variant, count FROM user_cards WHERE count>0')).rows.map((r) => ({ ...r, count: Number(r.count) })); }, // für Ranglisten: eine Abfrage statt einer je Spieler
    async addMinutes(ids, pokerIds) { if (ids.length) await db.query('UPDATE users SET play_minutes = COALESCE(play_minutes,0) + 1, poker_minutes = COALESCE(poker_minutes,0) + CASE WHEN id = ANY($2::int[]) THEN 1 ELSE 0 END WHERE id = ANY($1::int[])', [ids, pokerIds]); }, // Spielzeit für alle Online-Spieler in einem Schritt
    async usersByIds(ids) { if (!ids.length) return []; return (await db.query(`SELECT ${COLS} FROM users WHERE id = ANY($1::int[])`, [ids])).rows.map(num); }, // viele Spieler mit einer Abfrage
