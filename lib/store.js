...
const NUM_KEYS = ['matches', 'wins', 'answered', 'exact', 'close', 'mc_right', 'mc_total', 'points', 'dev_sum', 'dev_n', 'xp', 'prestige', 'best_score', 'streak', 'best_streak', 'av', 'rank_points', 'pres_shown', 'last_seen', 'diamonds', 'casino_xp', 'casino_rounds', 'casino_wins', 'casino_best', 'casino_net', 'daily_streak', 'daily_base', 'packs_opened', 'melted', 'casino_day_net', 'cash_prev_net', 'wheel_used'];
const TEXT_KEYS = ['tag', 'tag_color', 'emblem', 'title', 'codes', 'role', 'unlocks', 'frame', 'seen_items', 'daily_day', 'casino_day', 'cash_prev_day', 'cash_claimed', 'wheel_day', 'casino_theme'];
...
function pgStore(url) {
  ...
  return {
    ...
    async recentUsers(n) { return (await db.query(`SELECT ${COLS} FROM users ORDER BY id DESC LIMIT $1`, [n])).rows; },
    async resetWheel() { const r = await db.query('UPDATE users SET wheel_used=0 WHERE wheel_used>0'); return r.rowCount; },
    async casinoRanking() { return (await db.query(`SELECT ${COLS} FROM users WHERE casino_rounds>0 ORDER BY casino_xp DESC LIMIT 20`)).rows; },
