...
let CAP_K = [0, 2, 3, 4, 5, 8], CAP_W = [70, 50, 36, 24, 14, 7]; // Auszahlung ≈ 95,8 % (Simulation 3,6 Mio. Drehs)
...
function capShake(g, rnd = randomInt) { // Kappen werden Narren, dazu zufällig weitere Felder
  const caps = []; g.forEach((col, c) => col.forEach((x, r) => { if (x === 'N') caps.push([c, r]); }));
  if (!caps.length) return null;
  const turned = [];
  const extra = pickW(CAP_K, CAP_W, rnd), free = []; g.forEach((col, c) => col.forEach((x, r) => { if (x !== 'W' && x !== 'N') free.push([c, r]); }));
...
function play(bet, rnd = randomInt) {
  const grid = spinGrid(rnd), before = grid.map((c) => [...c]), cap = capShake(grid, rnd), r = evaluate(grid, bet);
