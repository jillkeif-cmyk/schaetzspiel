...
module.exports = function createPool(store) {
  ...
  async function autoCheck() { // alle Spieler der letzten 14 Tage, nur spielbare Fragen, gleicher Inhalt zählt als gesehen
    if (!fill.auto) return;
    try {
      ...
      for (const t of themes.THEMES) {
        ...
        let worst = list.length, who = 0;
        seenGroups.forEach((x, k) => { const un = list.filter((q) => !isSeen(q, x)).length; if (un < worst) { worst = un; who = k; } });
        if (worst < list.length * 0.2) { fill.targets[t.key] = Math.max(targetOf(t.key), playable(t.key) + 20); saveCfg(); note(`${t.name}: ${active[who].name} hat nur noch ${worst} von ${list.length} ungesehen, fülle nach (${active.length} aktive Spieler geprüft)`); start(); }
      }
