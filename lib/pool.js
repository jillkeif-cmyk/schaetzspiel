...
module.exports = function createPool(store) {
  ...
  function groupFor(q, probe) {
    ...
    for (const r of cand) if (sim.similar(r.q, q.q, r.a, q.a) || (q.t === 'mc' && r.t === 'mc' && String(r.o[0]).toLowerCase() === String(q.o[0]).toLowerCase() && sim.jaccard(sim.tokens(r.q), sim.tokens(q.q)) > 0.3)) return groupOf.get(r.id);
