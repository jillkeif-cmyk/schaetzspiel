...
module.exports = function createPool(store) {
  ...
  const isBad = (q) => sim.isClassic(q) || ai.leaks(q) || (q.t === 'mc' && (q.o || []).some((o) => String(o).length === 40)) || (q.t === 'est' && !ai.isQtyQuestion(q.q)); // Schätzfrage ohne Mengenfrage sperren
  function groupFor(q, probe) {
    ...
    for (const r of cand) if (sim.similar(r.q, q.q, r.a, q.a) || (q.t === 'mc' && r.t === 'mc' && String(r.o[0]).toLowerCase() === String(q.o[0]).toLowerCase() && (() => { const ta = sim.tokens(r.q), tb = sim.tokens(q.q); let ov = 0; for (const w of ta) if (tb.has(w)) ov++; return sim.jaccard(ta, tb) > 0.3 || ov >= 2; })())) return groupOf.get(r.id);
  ...
  function pick(themeKeys, seen, reported, total) {
    ...
    const repGroups = new Set([...reported].map((id) => groupOf.get(id)).filter(Boolean)); // gemeldet: auch Umformulierungen sperren
    const cand = shuffle(themeKeys.flatMap((k) => byTheme.get(k) || []).filter((q) => !q.bad && !seen.has(q.id) && !seenGroups.has(groupOf.get(q.id)) && !reported.has(q.id) && !repGroups.has(groupOf.get(q.id))));
  ...
  async function needReport(onlyIds) {
    const since = Date.now() - 14 * 86400000;
    let active = await store.activeUsers(since).catch((e) => { console.error('Bedarf: aktive Spieler', e.message); return []; });
    if (onlyIds && onlyIds.length) active = active.filter((u) => onlyIds.includes(u.id));
    const seenGroups = [];
    for (const u of active) { await new Promise((ok) => setImmediate(ok)); const seen = await store.seenCounts(u.id); const gs = new Set(); for (const id of seen.keys ? seen.keys() : seen) { const g = groupOf.get(id); if (g) gs.add(g); } seenGroups.push({ seen, gs }); }
    const isSeen = (q, x) => x.seen.has(q.id) || x.gs.has(groupOf.get(q.id)); // Gruppe nur nachschlagen (beim Laden berechnet), nicht neu suchen: vorher blockierte das den Server für Sekunden
    const rows = [];
    for (const t of themes.THEMES) {
      if (SKIP.has(t.key)) continue;
      await new Promise((ok) => setImmediate(ok)); // zwischen den Kategorien andere Anfragen durchlassen
  ...
  async function fillNeeded(onlyIds) { // wo es knapp ist: so viel nachbestellen, dass der Vielspieler danach wieder ≥ 30 % neue Fragen hat
    const r = await needReport(onlyIds); let n = 0;
    for (const row of r.rows) if (row.need) {
      const add = Math.max(20, Math.ceil((0.3 * row.count - row.worst) / 0.7)); // nach +add: (worst+add)/(count+add) ≥ 30 %
      const want = playable(row.key) + add;
      if (want <= targetOf(row.key) && fill.running) continue; // läuft schon auf ein ausreichendes Ziel zu
      fill.targets[row.key] = Math.max(targetOf(row.key), want); n++; note(`${row.name}: ${row.who} hat nur noch ${row.worst} von ${row.count} neu, bestelle ${add} nach`);
    }
    if (n) { saveCfg(); start(); }
    return { ...r, raised: n };
  }
  ...
  setInterval(autoCheck, 5 * 60 * 1000); // regelmäßig alle aktiven Spieler prüfen
  let afterT = null, afterIds = new Set();
  function afterMatch(ids) { // nach einem Match gezielt die Mitspieler prüfen (gesammelt, 45 s später)
    if (!fill.auto) return; for (const id of ids || []) afterIds.add(id);
    clearTimeout(afterT); afterT = setTimeout(async () => { const list = [...afterIds]; afterIds = new Set(); try { const r = await fillNeeded(list); if (r.raised) note(`Nach Match: ${r.raised} Kategorien für ${list.length} Mitspieler knapp`); } catch (e) { console.error('Pool nach Match:', e.message); } }, 45000);
  }
  ...
  return { afterMatch, get: (id) => byId.get(id), needReport, fillNeeded, addLive, setThemeTarget, seenContent, setMode, setEffort, load, add, pick, start, stop, stats, setTarget, setAuto, themeOfQ, size: (k) => (byTheme.get(k) || []).length };
};
