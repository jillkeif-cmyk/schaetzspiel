...
module.exports = function createPool(store) {
  ...
  async function needReport() {
    ...
    const active = await store.activeUsers(since).catch((e) => { console.error('Bedarf: aktive Spieler', e.message); return []; });
    const seenGroups = [];
  ...
  return { get: (id) => byId.get(id), needReport, fillNeeded, addLive, setThemeTarget, seenContent, setMode, setEffort, load, add, pick, start, stop, stats, setTarget, setAuto, themeOfQ, size: (k) => (byTheme.get(k) || []).length };
};
