...
module.exports = function createPool(store) {
  ...
  // ---------- Befüllung ----------
  const fill = { mode: 'batch', effort: 'low', batch: null, batchStatus: '', running: false, auto: false, target: 200, inFlight: 0, made: 0, rejected: 0, startTokens: null, log: [], queue: new Set() };
  ...
  async function batchCycle() {
    ...
    for (const t of themes.THEMES) {
      if (SKIP.has(t.key)) continue;
      const missing = fill.target - byTheme.get(t.key).length;
  ...
  function stats(seenByMe) {
    ...
    return {
      loaded, distinct: reps.length, blocked: badCount, mode: fill.mode, effort: fill.effort, batchStatus: fill.batch ? (fill.batchStatus || 'abgeschickt') : '', running: fill.running, auto: fill.auto, target: fill.target, inFlight: fill.inFlight, made: fill.made, rejected: fill.rejected,
      ...
      themes: themes.THEMES.map((t) => {
        ...
        return { key: t.key, name: t.name, count: list.filter((q) => !q.bad).length, seen: seenByMe ? list.filter((q) => seenByMe.has(q.id)).length : 0, skip: SKIP.has(t.key) };
      }),
      log: fill.log,
    };
  }
  ...
  function setMode(m) { if (['batch', 'direct'].includes(m)) { fill.mode = m; note(m === 'batch' ? 'Modus: Sammelauftrag (halbe Gebühr, langsamer)' : 'Modus: direkt (schneller, volle Gebühr)'); } }
  function setEffort(e) { if (['low', 'medium', 'high'].includes(e)) { fill.effort = e; note('Nachdenk-Stufe: ' + e); } }
  return { seenContent, setMode, setEffort, load, add, pick, start, stop, stats, setTarget, setAuto, themeOfQ, size: (k) => (byTheme.get(k) || []).length };
};
