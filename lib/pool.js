...
module.exports = function createPool(store) {
  ...
  async function load() {
    try { add(await store.poolAll()); loaded = true; console.log('Fragen-Pool geladen:', [...byTheme.values()].reduce((x, l) => x + l.length, 0), 'Fragen'); }
  ...
  async function loop() {
    while (fill.running) {
      ...
      if (!nextTheme() && !fill.inFlight) { fill.running = false; note('Alle Kategorien haben den Zielstand erreicht'); break; }
      await new Promise((r) => setTimeout(r, 1500));
    }
  }
  function start() { if (fill.running || !ai.enabled()) return; fill.running = true; if (!fill.startTokens) fill.startTokens = { ...ai.usage }; note(`Befüllung gestartet, Ziel ${fill.target} pro Kategorie`); loop(); }
  function stop() { if (!fill.running) return; fill.running = false; note('Befüllung angehalten'); }
