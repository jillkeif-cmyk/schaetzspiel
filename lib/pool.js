...
module.exports = function createPool(store) {
  ...
  async function autoCheck() {
    ...
    try {
      ...
      for (const t of themes.THEMES) {
        ...
        if (worst < list.length * 0.2) { fill.targets[t.key] = Math.max(targetOf(t.key), playable(t.key) + 20); saveCfg(); note(`${t.name}: nur noch ${worst} ungesehen, fülle nach`); start(); }
      }
  ...
  return { addLive, setThemeTarget, seenContent, setMode, setEffort, load, add, pick, start, stop, stats, setTarget, setAuto, themeOfQ, size: (k) => (byTheme.get(k) || []).length };
};
