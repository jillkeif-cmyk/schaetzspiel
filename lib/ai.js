...
[{"t":"est","q":"...","a":123.4,"unit":"m","cat":"Bauwerke"},{"t":"mc","q":"...","o":["A","B","C","D"],"c":2,"cat":"Kultur"}]`;
  ...
  try {
    const res = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        model: MODEL,
        max_tokens: Math.min(16000, 900 + count * 320),
        // Die Antwort startet direkt mit dem Array, so kann kein Vortext entstehen
    ...
    if (!text.includes('[')) text = '[' + text; // vorgefüllte Klammer wieder ergänzen
    let out;
