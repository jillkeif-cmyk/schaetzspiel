...
async function generate(count, avoid = [], opts = {}) {
  if (MOCK && !KEY) {
    await new Promise((r) => setTimeout(r, MOCK));
    const base = Array.from({ length: count }, (_, i) => ({ t: 'est', q: `Testfrage ${Date.now()}-${i}: Wie viele Meter misst Objekt ${Math.random().toString(36).slice(2, 7)}?`, a: 100 + i, unit: 'm', cat: 'Test' }));
  ...
  if (count > CHUNK) {
    ...
    const results = await Promise.all(Array.from({ length: parts }, (_, i) => {
      ...
      const ts = Date.now();
      return generateOne(n, avoid, { ...opts, variant: i })
        .then((r) => { console.log(`KI-Häppchen ${i + 1}/${parts}: ${r.length} Fragen in ${Math.round((Date.now() - ts) / 1000)} s`); return r; })
  ...
  return generateOne(count, avoid, opts);
}
...
${avoid.slice(0, 120).map((t) => '  - ' + String(t).slice(0, 90)).join('\n')}${vary}
...
[{"t":"est","q":"...","a":123.4,"unit":"m","cat":"Bauwerke"},{"t":"mc","q":"...","o":["A","B","C","D"],"c":2,"cat":"Kultur"}]`;
  ...
  try {
    const res = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        model: MODEL,
        max_tokens: Math.min(20000, 3000 + count * 500),
