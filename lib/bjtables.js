...
module.exports = function setupTables(io, store, onStat, push, isLocked = () => false) {
  ...
  function attach(socket, user) {
    ...
    socket.on('bj:create', () => {
      if (isLocked()) return socket.emit('toast', 'Blackjack-Tische sind gerade gesperrt.');
  ...
  return { attach, tables, leave, isSeated: (id) => seatOf.has(id) };
};
