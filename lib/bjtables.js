...
module.exports = function setupTables(io, store, onStat) {
  ...
  const view = (t) => ({
    id: t.id, hostId: t.hostId, phase: t.phase, max: MAX_SEATS, turn: t.turn,
    ...
    seats: t.seats.map((s) => {
      ...
      return { id: s.id, name: s.name, bet: t.bets.get(s.id) || 0, payout: t.payouts.get(s.id) || 0,
  ...
  function resetRound(t) { t.phase = 'betting'; t.bets = new Map(); t.games = new Map(); t.payouts = new Map(); t.dealer = null; t.turn = null; }
  ...
  function attach(socket, user) {
    ...
    socket.on('bj:join', (id) => {
      ...
      t.seats.push({ id: user.id, name: user.name, sockets: new Set([socket]) });
    ...
    socket.on('bj:bet', async (raw) => {
      try {
        ...
        if (amount < 50 || amount > 20000) return fail('Einsatz zwischen 50 und 20.000.');
        ...
        t.bets.set(user.id, amount); socket.emit('diamonds', have - amount); pushTable(t);
