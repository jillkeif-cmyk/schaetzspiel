...
// Drei Chips auf einer Linie geben einen Gratis-Dreh. Exakt berechnete Auszahlungsquote: 96,0 %, Vollbild etwa 1 zu 614 (nah am Original).
// Vorher (bis 24.09.2026): Vollbild 1 zu 6.097, Krone 150, Hufeisen 12, Auszahlung 96,2 %.
const { randomInt } = require('crypto');
const PAY = { K: 200, S: 40, H: 20, B: 8, R: 8, T: 8, M: 8, C: 1 }; // Vielfaches des Einsatzes je Linie
...
const STRIPS = ['KCCBBBBBHCRRRRSCTTTTCMMMMCHCSC', 'CCSBBBBCKCRRRRRCTTTTCMMMMCHCSC', 'CKCBBBBCHCRRRRCSTTTTTCMMMMCHSC'].map((s) => s.split('')); // Obst in 4er- und 5er-Blöcken: Vollbilder deutlich häufiger
...
function play(bet, forceFull = false) { // forceFull: nur für den Admin zum Testen, erster Dreh ist ein Vollbild
  const spins = []; let total = 0, respins = 0;
  for (;;) {
    const fs = forceFull && !spins.length ? [...FRUIT][randomInt(4)] : null;
    const grid = fs ? [[fs, fs, fs], [fs, fs, fs], [fs, fs, fs]] : spinGrid(), r = evaluate(grid, bet);
    let wheel = null;
