...
// Risikoleiter wie am Automaten: Es blinken abwechselnd eine höhere und eine tiefere Stufe.
// Unten hüpft es zwischen Nachbarstufen, weiter oben fällt man tiefer zurück. Mitten in der Leiter
// sitzt die AUSSPIELUNG: Wer dort landet, wird ausgespielt, der Betrag wird ausgezahlt und das
// Risiko ist vorbei. Die Chance ist immer fair: (aktuell - unten) / (oben - unten).
const nice = (v) => (v < 100 ? Math.round(v) : v < 1000 ? Math.round(v / 5) * 5 : v < 10000 ? Math.round(v / 10) * 10 : Math.round(v / 100) * 100);
function ladder(win) {
  const down = []; let v = win;
  for (let k = 0; k < 3; k++) { v = nice(v / 2); if (v < 1 || (down[0] && v >= down[0])) break; down.unshift(v); }
  const steps = [{ v: 0 }, ...down.map((x) => ({ v: x })), { v: win }];
  let cur = win;
  for (let k = 0; k < 7; k++) { cur = Math.max(cur + 1, nice(cur * 1.6)); steps.push(k === 2 ? { v: cur, aus: true } : { v: cur }); }
  return { steps, pos: steps.length - 8 };
}
// Wie tief fällt man? Unten eine Stufe, weiter oben mehr - so hüpft es oben zwischen hoch und weit unten
const lowIndex = (pos) => Math.max(0, pos - (1 + Math.floor(pos / 3)));
...
module.exports = { play, evaluate, ladder, lowIndex, chance, riskWins, drawCard, cardRed, PAY, NAMES, LINES, STRIPS, BETS };
