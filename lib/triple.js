...
// Risikoleiter wie am Automaten: Beim Riskieren springt der Zeiger zwischen der nächsten Stufe nach
// oben und dem Feld darunter. Ganz unten steht die AUSSPIELUNG: Wer dort landet, verliert den
// kompletten Gewinn, das Risiko ist vorbei. Man muss das Risiko nicht annehmen, Nehmen geht immer.
...
function ladder(win) {
  ...
  let cur = win;
  for (let k = 0; k < 7; k++) { cur = Math.max(cur + 1, nice(cur * 1.6)); steps.push({ v: cur }); }
  return { steps, pos: 1 };
}
