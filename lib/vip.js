...
function xpFor(risk, won, stake) { // Grundwert nach Einsatz + Zuschlag nach Gewinnfaktor + 1 % des Reingewinns bei großen Gewinnen (ab 5×)
  ...
  const big = m >= 5 ? Math.round((won - stake) * 0.01) : 0; // 10 Mio. Gewinn ≈ 100.000 XP
  return Math.min(300000, base + Math.round(base * f) + big);
}
