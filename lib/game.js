...
module.exports = function attach(io, store) {
  ...
  async function prepareQuestions(m) {
    ...
    const fresh_n = Math.ceil(m.questions.length * (m.waitAi ? 0.85 : 0.4));
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('settings', (s) => {
      ...
      if (typeof s.ranked === 'boolean') { m.ranked = s.ranked; m.themes = m.ranked ? [...themes.RANKED] : themes.valid(m.themes.slice(0, themes.MAX_THEMES)); pushLobbies(); }
