...
module.exports = function attach(io, store) {
  ...
  function stateFor(m, uid) {
    ...
    return {
      code: m.code, phase: m.phase, hostId: m.hostId, you: uid, settings: m.settings, locked: !!m.password, ranked: m.ranked, waitAi: m.waitAi, setup: m.setup, chat: m.chat, themes: m.themes, themeList: themes.list(), maxThemes: themes.MAX_THEMES, password: uid === m.hostId ? m.password : undefined,
      ...
      players: [...m.players.values()].map((p) => ({ id: p.id, name: p.name, color: p.color, level: p.level, prestige: p.prestige, presShown: p.presShown, av: p.av, tag: p.tag, tagColor: p.tagColor, emblem: p.emblem, title: p.title, streak: p.streak, score: p.score, connected: p.sockets.size > 0, answered: p.answer !== null, gain: revealed ? p.gain : 0 })),
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('create', () => {
      ...
      const m = { code: newCode(), phase: 'lobby', hostId: user.id, players: new Map(), settings: { ...DEFAULTS }, questions: [], qIndex: 0, current: null, reveal: null, endsAt: null, aiStatus: ai.enabled() ? 'idle' : 'off', reportedBy: new Set(), winners: null, password: '', summaries: {}, ranked: false, themes: ALL_THEMES, setup: true, chat: [], asked: new Set(), invited: new Set(), waitAi: false };
    ...
    async function join(m) {
      ...
      m.players.set(user.id, { id: user.id, name: user.name, color: COLORS.find((c) => !used.has(c)), score: 0, gain: 0, answer: null, sockets: new Set([socket]),
        ...
        level: progress.levelInfo(u.xp || 0).level, prestige: u.prestige || 0, presShown: (u.pres_shown === -1 ? 0 : u.pres_shown > 0 ? Math.min(u.pres_shown, u.prestige || 0) : (u.prestige || 0)), av: u.av || 0, tag: u.tag || '', tagColor: u.tag_color || '', emblem: u.emblem || '', streak: u.streak || 0,
    ...
    socket.on('settings', (s) => {
      ...
      if (typeof s.waitAi === 'boolean') m.waitAi = s.waitAi;
    ...
    socket.on('challenge', (targetId) => {
      const m = mine(), id = Number(targetId);
      if (!m || m.setup || !['lobby', 'countdown'].includes(m.phase)) return fail('Eröffne zuerst ein Match, dann kannst du jemanden herausfordern.');
      if (m.players.size >= MAX_PLAYERS) return fail('Dein Match ist voll.');
      const t = online.get(id);
      if (!t) return fail('Dieser Spieler ist gerade nicht online.');
      if (userMatch.has(id)) return fail(t.name + ' ist schon in einem Match.');
      m.invited.add(id);
      for (const s of t.sockets) s.emit('challenged', { from: user.name, code: m.code });
      fail('Herausforderung an ' + t.name + ' geschickt.');
    });
