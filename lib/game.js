...
module.exports = function attach(io, store) {
  ...
  function stateFor(m, uid) {
    ...
    return {
      ...
      players: [...m.players.values()].map((p) => ({ id: p.id, name: p.name, color: p.color, level: p.level, prestige: p.prestige, av: p.av, tag: p.tag, tagColor: p.tagColor, emblem: p.emblem, title: p.title, score: p.score, connected: p.sockets.size > 0, answered: p.answer !== null, gain: revealed ? p.gain : 0 })),
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('create', () => {
      ...
      const m = { code: newCode(), phase: 'lobby', hostId: user.id, players: new Map(), settings: { ...DEFAULTS }, questions: [], qIndex: 0, current: null, reveal: null, endsAt: null, aiStatus: ai.enabled() ? 'idle' : 'off', reportedBy: new Set(), winners: null, password: '', summaries: {}, ranked: false, themes: ALL_THEMES, setup: true, chat: [] };
    ...
    async function join(m) {
      ...
      m.players.set(user.id, { id: user.id, name: user.name, color: COLORS.find((c) => !used.has(c)), score: 0, gain: 0, answer: null, sockets: new Set([socket]),
        ...
        level: progress.levelInfo(u.xp || 0).level, prestige: u.prestige || 0, av: u.av || 0, tag: u.tag || '', tagColor: u.tag_color || '', emblem: u.emblem || '',
        title: (() => { const t = cards.titleById(u.title); return t && cards.has(t, u) ? { text: t.text, style: t.style } : null; })() });
