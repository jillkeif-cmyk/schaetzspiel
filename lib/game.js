...
const WAIT_EXTRA_MS = 120000; // so lange darf auf frische Fragen zusätzlich gewartet werden
...
module.exports = function attach(io, store) {
  ...
  async function prepareQuestions(m) {
    ...
    ai.generate(fresh_n, m.questions.map((q) => q.q), { themes: themeList, timeout: m.waitAi ? 170000 : Math.max(15000, m.settings.countdown * 1000 - 4000) }).then(async (raw) => {
