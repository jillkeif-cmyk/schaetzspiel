...
module.exports = function attach(io, store) {
  ...
  async function prepareQuestions(m) {
    ...
    run(1).then(async (raw) => {
      ...
      if (!stored.length) { m.aiStatus = 'failed'; push(m); if (m.phase === 'countdown' && m.startWhenReady) timer(m, 1500, () => nextQuestion(m)); return; }
      if (m.phase !== 'lobby' && m.phase !== 'countdown') return; // Spiel läuft schon, nichts mehr tauschen
      // Nur frische Fragen, kein Pool. Kamen weniger als gewünscht, wird die Runde eben etwas kürzer.
      m.questions = shuffle(stored).slice(0, total);
      for (const q of m.questions) m.asked.add(q.id);
