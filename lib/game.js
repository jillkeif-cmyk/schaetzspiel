...
module.exports = function attach(io, store) {
  ...
  async function prepareQuestions(m) {
    ...
    if (!ai.enabled() || (m.themes.length === 1 && m.themes[0] === 'grace')) { m.aiStatus = 'off'; return; }
  ...
  function startCountdown(m) {
    m.phase = 'countdown'; m.qIndex = 0; m.current = null; m.reveal = null; m.winners = null; m.drops = [];
  ...
  function finish(m) {
    ...
    Promise.all(ps.filter((p) => p.match).map(async (p) => {
      ...
      if (solo && (r.answered || 0) > 0) {
        const cut = Math.round((out.summary.gained || 0) * 0.35);
        out.user.xp = Math.max(0, (out.user.xp || 0) - cut);
        out.summary.parts = [...(out.summary.parts || []), ['Solo-Runde (−35 %)', -cut]];
