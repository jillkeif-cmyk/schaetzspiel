...
module.exports = function attach(io, store) {
  ...
  async function prepareQuestions(m) {
    ...
    const run = async (n) => {
      try {
        const r = await ai.generate(fresh_n, [...m.questions.map((q) => q.q), ...lastAsked], { themes: themeList, timeout: m.waitAi ? 230000 : Math.max(15000, m.settings.countdown * 1000 - 4000) });
        if (r && r.length) return r;
      } catch (e) { console.error('KI-Versuch ' + n + ' fehlgeschlagen:', e.message); }
      return n < tries ? run(n + 1) : [];
    };
  ...
  io.on('connection', (socket) => {
    ...
    const newMatch = () => ({ code: newCode(), phase: 'lobby', hostId: user.id, players: new Map(), settings: { ...DEFAULTS }, questions: [], qIndex: 0, current: null, reveal: null, endsAt: null, aiStatus: ai.enabled() ? 'idle' : 'off', reportedBy: new Set(), winners: null, password: '', summaries: {}, ranked: false, themes: defaultThemes(), setup: true, chat: [], asked: new Set(), invited: new Set(), waitAi: false });
    ...
    socket.on('again', () => {
      ...
      clearTimeout(m.timer); m.phase = 'lobby'; m.setup = false; m.prepDone = false; m.aiStatus = ai.enabled() ? 'idle' : 'off'; m.questions = []; m.current = null; m.reveal = null; m.winners = null; m.endsAt = null; m.summaries = {}; m.qIndex = 0;
