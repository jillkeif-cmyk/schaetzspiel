...
module.exports = function attach(io, store) {
  ...
  function activityOf(id) {
    ...
    if (m) {
      const kind = m.ranked ? 'Rangliste' : m.password ? 'private Runde' : 'offene Runde';
  ...
  function finish(m) {
    ...
    Promise.all(ps.filter((p) => p.match).map(async (p) => {
      ...
      let dia = tcg.diamondsFor({ win: r.win, score: r.score, pointLimit: m.settings.pointLimit, counted: r.counted });
