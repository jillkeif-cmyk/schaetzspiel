...
module.exports = function attach(io, store) {
  ...
  function stateFor(m, uid, watching = false) {
    ...
    return {
      ...
      aiStatus: m.aiStatus, waitingMore: !!m.waitingMore, startWhenReady: !!m.startWhenReady, readyGo: !!m.readyGo, qIndex: m.qIndex, qTotal: m.questions.length || m.settings.maxQuestions,
  ...
  async function prepareQuestions(m) {
    ...
    if ((await store.setting('question_source')) === 'pool') {
      ...
      if (have.length >= START_MIN) {
        m.questions = have.slice(0, total); for (const q of m.questions) m.asked.add(q.id);
    ...
    const take = (list) => { chain = chain.then(async () => {
      ...
      const ok = await fresh_ok(diverse.filter((q) => themeMatch(q) && !have.some((x) => sim.similar(x.q, q.q, x.a, q.a))));
      if (!ok.length) return;
      ...
      qpool.add(stored.filter((q) => String(q.id).startsWith('a'))); // Live-Fragen landen automatisch im Pool
      have = [...have, ...stored];
    ...
    const run = async (n) => {
      ...
      try {
        ...
        await ai.generate(ask, [...lastAsked.slice(-30).reverse(), ...known.slice(0, 40)], { themes: themeList, timeout: 150000, onChunk: take });
        await chain;
  ...
  return { matches, scoreEstimate, online, userMatch, activityOf, hooks, qpool };
};
