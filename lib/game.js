...
function scoreEstimate(q, guess) {
  ...
  if (d < 1e-9) return { points: 1000, exact: true, close: false, d };
...
module.exports = function attach(io, store) {
  ...
  function lobbies() {
    return {
      ...
      open: [...matches.values()].map((m) => ({ code: m.code, host: m.players.get(m.hostId)?.name || '?', players: m.players.size, phase: m.phase, locked: !!m.password, qIndex: m.qIndex, qTotal: m.questions.length || m.settings.maxQuestions })),
    };
  }
  ...
  function stateFor(m, uid) {
    ...
    return {
      code: m.code, phase: m.phase, hostId: m.hostId, you: uid, settings: m.settings, locked: !!m.password, password: uid === m.hostId ? m.password : undefined,
  ...
  async function prepareQuestions(m) {
    ...
    let cand = shuffle([...pool, ...saved].filter((q) => !skip.has(q.id)));
  ...
  function finish(m) {
    ...
    Promise.all(ps.filter((p) => p.match).map(async (p) => {
      const r = { ...p.match, score: p.score, win: ps.length > 1 && m.winners.includes(p.id), counted: ps.length > 1 }; p.match = null;
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('create', () => {
      ...
      const m = { code: newCode(), phase: 'lobby', hostId: user.id, players: new Map(), settings: { ...DEFAULTS }, questions: [], qIndex: 0, current: null, reveal: null, endsAt: null, aiStatus: ai.enabled() ? 'idle' : 'off', reportedBy: new Set(), winners: null, password: '', summaries: {} };
