...
module.exports = function attach(io, store) {
  ...
  function finish(m) {
    ...
    store.ctx.run({ src: 'Quiz-Match' }, () => Promise.all(ps.filter((p) => p.match).map(async (p) => {
      const sw = solowin.get(), soloWin = ps.length === 1 && sw.on && p.score >= sw.min && m.qIndex <= sw.maxQ; // Solo-Sieg: genug Punkte in wenigen Fragen
      ...
      if (quizpay.get().on && (r.answered || 0) > 0 && (solo || r.counted)) dia = quizpay.amount({ solo, win: r.win, score: r.score }); // neue Quiz-Belohnung
