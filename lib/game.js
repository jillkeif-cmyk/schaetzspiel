...
module.exports = function attach(io, store) {
  ...
  function stateFor(m, uid) {
    ...
    return {
      code: m.code, phase: m.phase, hostId: m.hostId, you: uid, maxPlayers: MAX_PLAYERS, settings: m.settings, locked: !!m.password, ranked: m.ranked, waitAi: m.waitAi, setup: m.setup, chat: m.chat, themes: m.themes, themeList: themes.list(), maxThemes: themes.MAX_THEMES, password: uid === m.hostId ? m.password : undefined,
  ...
  io.on('connection', (socket) => {
    ...
    const newMatch = () => ({ code: newCode(), phase: 'lobby', hostId: user.id, players: new Map(), settings: { ...DEFAULTS }, questions: [], qIndex: 0, current: null, reveal: null, endsAt: null, aiStatus: ai.enabled() ? 'idle' : 'off', reportedBy: new Set(), winners: null, password: '', summaries: {}, ranked: false, themes: ALL_THEMES, setup: true, chat: [], asked: new Set(), invited: new Set(), waitAi: false });
