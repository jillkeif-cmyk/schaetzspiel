...
module.exports = function attach(io, store) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('create', () => {
      ...
      const m = { code: newCode(), phase: 'lobby', hostId: user.id, players: new Map(), settings: { ...DEFAULTS }, questions: [], qIndex: 0, current: null, reveal: null, endsAt: null, aiStatus: ai.enabled() ? 'idle' : 'off', reportedBy: new Set(), winners: null, password: '', summaries: {}, ranked: false, themes: ALL_THEMES, setup: true, chat: [], asked: new Set(), invited: new Set() };
