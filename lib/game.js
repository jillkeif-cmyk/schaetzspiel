...
const kpass = require('./pass');
const cards = require('./cards');
const frames = require('./frames');

const MAX_MATCHES = Number(process.env.MAX_MATCHES || 4);
const MAX_PLAYERS = 8;
const REVEAL_MS = Number(process.env.REVEAL_MS || 7000); // Auflösung im Mehrspieler (vorher 9 s)
const REVEAL_SOLO_MS = Number(process.env.REVEAL_SOLO_MS || 5500); // Auflösung im Solo
const solowin = require('./solowin');
const quizpay = require('./quizpay');
...
const DEFAULTS = { pointLimit: 2000, maxQuestions: 25, countdown: 60, answerTime: 20 }; // = „Standard“ in der App (PRESET)
...
// Punkte für eine Schätzung. Punktlandung 300, Doppel-Treffer 200 (höchstens 8 % neben der Lösung, bei Jahreszahlen je nach Epoche ±4 bis ±30 Jahre), sonst 0-100 nach Nähe.
function scoreEstimate(q, guess) {
  ...
  if (d <= range * 0.2) return { points: 200, exact: false, close: true, d }; // ein Fünftel des Wertungsbereichs = 8 % der Lösung
...
module.exports = function attach(io, store) {
  ...
  const PAGE_TXT = { home: 'im Hauptmenü', casino: 'im Casino', 'casino:roulette': 'spielt Roulette', 'casino:bj': 'spielt Blackjack', 'casino:vip': 'am Glücksrad', 'casino:jester': 'spielt Narrenkappe', 'casino:anubis': 'spielt Auge des Anubis', 'casino:crypt': 'spielt Buch der Gruft', 'casino:rewards': 'schaut Casino-Belohnungen', 'casino:crank': 'schaut die Casino-Rangliste', 'casino:tables': 'sucht einen Blackjack-Tisch', 'casino:poker': 'schaut beim Poker zu', 'casino:triple': 'bei Triple Crown', 'casino:watch': 'schaut im Casino zu', cards: 'bei Karten & Booster', ranks: 'schaut die Ranglisten', profile: 'bearbeitet sein Profil' };
  ...
  function lobbies() {
    return {
      ...
      open: [...matches.values()].filter((m) => !m.solo || (m.phase !== 'lobby' && m.phase !== 'finished')).map((m) => ({ code: m.code, host: m.players.get(m.hostId)?.name || '?', players: m.players.size, phase: m.phase, setup: m.setup, locked: !!m.password, ranked: m.ranked, solo: !!m.solo, qIndex: m.qIndex, qTotal: m.settings.maxQuestions })),
    };
  }
  ...
  function stateFor(m, uid, watching = false) {
    ...
    return {
      ...
      aiStatus: m.aiStatus, fromPool: !!m.fromPool, waitingMore: !!m.waitingMore, startWhenReady: !!m.startWhenReady, readyGo: !!m.readyGo, qIndex: m.qIndex, qTotal: m.settings.maxQuestions,
  ...
  // Fehlen Fragen (KI hat zu wenig geliefert), aus dem Fragenpool auffüllen, notfalls aus den eingebauten Fragen
  async function topUp(m) {
    const total = m.settings.maxQuestions, need = total - m.questions.length; if (need <= 0) return 0;
    let add = [];
    try {
      const keys = m.ranked ? themes.RANKED : m.themes;
      const seen = await store.seenOf([...m.players.keys()]).catch(() => new Set()), rep = await store.reportedIds().catch(() => new Set());
      add = qpool.pick(keys, seen, rep, need * 2).filter((q) => !m.asked.has(q.id) && !m.questions.some((x) => x.id === q.id));
    } catch (e) { console.error('Auffüllen (Pool):', e.message); }
    if (add.length < need) add = [...add, ...shuffle([...pool]).filter((q) => !m.asked.has(q.id) && !m.questions.some((x) => x.id === q.id) && !add.some((x) => x.id === q.id))];
    add = add.slice(0, need);
    for (const q of add) { m.questions.push(q); m.asked.add(q.id); }
    if (add.length) console.log(`Fragen aufgefüllt: +${add.length}, jetzt ${m.questions.length} von ${total} (${m.solo ? 'Solo' : 'Match'} ${m.code})`);
    return add.length;
  }
  async function prepareQuestions(m) {
    ...
    run(1).then(async () => {
      await chain;
      if (matches.has(m.code) && m.phase !== 'finished' && m.questions.length >= START_MIN) await topUp(m); // zu wenig frische Fragen: auffüllen
      m.fetching = false;
  ...
  function startCountdown(m) {
    m.phase = 'countdown'; m.qIndex = 0; m.current = null; m.reveal = null; m.winners = null; m.drops = []; m.readyGo = false; m.startWhenReady = !!m.solo; m.noMore = false; m.toppedUp = false;
    if (m.solo && solowin.get().on) m.settings.pointLimit = Math.max(m.settings.pointLimit, solowin.get().min); // Solo endet nicht, bevor der Sieg möglich ist
  ...
  function nextQuestion(m) {
    ...
    if (!m.questions.length) { m.questions = shuffle([...pool]).slice(0, Number(process.env.TEST_QCAP) || m.settings.maxQuestions); for (const q of m.questions) m.asked.add(q.id); } // TEST_QCAP nur für Tests: zu wenig Fragen nachstellen
    const src = m.questions[m.qIndex];
    if (!src && !m.fetching && m.qIndex < m.settings.maxQuestions && !m.noMore) { if (m.toppedUp) return; m.toppedUp = true; topUp(m).then((n) => { m.toppedUp = false; if (!n) m.noMore = true; nextQuestion(m); }).catch(() => { m.toppedUp = false; m.noMore = true; nextQuestion(m); }); return; } // nie vorzeitig enden: erst auffüllen
  ...
  function endQuestion(m) {
    ...
    const over = top >= m.settings.pointLimit || m.qIndex >= m.settings.maxQuestions || (m.qIndex >= m.questions.length && !m.fetching && m.noMore);
    timer(m, m.solo ? REVEAL_SOLO_MS : REVEAL_MS, () => (over ? finish(m) : nextQuestion(m)));
    push(m);
  }
  ...
  function finish(m) {
    ...
    try { qpool.afterMatch(ps.map((p) => p.id)); } catch (e) {} // haben Mitspieler bald alles gesehen? dann Pool nachfüllen
    ...
    push(m); pushLobbies();
    store.ctx.run({ src: 'Quiz-Match' }, () => Promise.all(ps.filter((p) => p.match).map(async (p) => {
      const sw = solowin.get(), soloWin = ps.length === 1 && sw.on && p.score >= Math.round(solowin.need(m.settings.maxQuestions) * Math.min(1, m.qIndex / m.settings.maxQuestions)) && m.qIndex <= sw.maxQ; // weniger Fragen gespielt: Grenze anteilig // Solo-Sieg: genug Punkte in wenigen Fragen
      const r = { ...p.match, score: p.score, win: (ps.length > 1 && m.winners.includes(p.id)) || soloWin, counted: ps.length > 1 || (ps.length === 1 && sw.on),
      ...
      const u0 = { ...u }; // Stand beim Lesen, für die Differenz beim Speichern
      ...
      if (solo && (r.answered || 0) > 0) {
        ...
        out.user.xp = Math.min(progress.capFor(u.prestige), Math.max(0, (Number(u.xp) || 0) + (out.summary.gained || 0) - cut)); // erst abziehen, dann deckeln: auf Level 30 bleibt man auf Level 30
        out.summary.parts = [...(out.summary.parts || []), ['Solo-Runde (−12 %)', -cut]];
        out.summary.gained = (out.summary.gained || 0) - cut;
        out.summary.levelAfter = progress.levelInfo(out.user.xp, u.prestige).level;
      }
      // Pass-XP: nur die normale Match-XP, ohne Herausforderungen und ohne Verdopplung (erstes Match, Hot Time)
      const passBase = Math.max(0, (out.summary.gained || 0) - (out.summary.parts || []).filter((x) => String(x[0]).startsWith('Herausforderung')).reduce((a, x) => a + x[1], 0));
      if (soloWin) out.summary.soloWin = true;
      let dia = solo ? ((r.answered || 0) > 0 ? Math.round(tcg.diamondsFor({ win: soloWin, score: r.score, pointLimit: m.settings.pointLimit, counted: true }) * 0.7) : 0)
        : tcg.diamondsFor({ win: r.win, score: r.score, pointLimit: m.settings.pointLimit, counted: r.counted });
      if (quizpay.get().on && (r.answered || 0) > 0 && (solo || r.counted)) dia = quizpay.amount({ solo, win: r.win, score: r.score, need: solo ? solowin.need(m.settings.maxQuestions) : null }); // neue Quiz-Belohnung
      ...
      if (u.first_game_day !== today && (r.answered || 0) > 0) {
        ...
        out.user.xp = Math.min(progress.capFor(u.prestige), (out.user.xp || 0) + extra);
      ...
      // Doppel-XP-Aktion vom Admin (unabhängig von der Hot Time)
      if (boost.get().xp && (r.answered || 0) > 0) {
        const extra = Math.max(0, passBase);
        out.user.xp = Math.min(progress.capFor(u.prestige), (out.user.xp || 0) + extra);
        out.summary.parts = [...(out.summary.parts || []), ['Doppel-XP-Aktion ⚡', extra]];
        out.summary.gained = (out.summary.gained || 0) + extra;
        out.summary.levelAfter = progress.levelInfo(out.user.xp, u.prestige).level;
      }
      // Trank für Level-XP (×2 / ×4)
      const pm = potions.mult(p.id, 'xp');
      if (pm > 1 && (r.answered || 0) > 0) {
        const extra = Math.max(0, passBase) * (pm - 1);
        out.user.xp = Math.min(progress.capFor(u.prestige), (out.user.xp || 0) + extra);
        out.summary.parts = [...(out.summary.parts || []), [`Trank ×${pm} 🧪`, extra]];
        out.summary.gained = (out.summary.gained || 0) + extra;
        out.summary.levelAfter = progress.levelInfo(out.user.xp, u.prestige).level;
      }
      // Kronen-Pass: Match-XP zählen als Pass-XP (ohne Herausforderungen), auch auf Level 30
      if (kpass.active() && (r.answered || 0) > 0) {
        const gain = passBase * (boost.get().pxp ? 2 : 1) * potions.mult(p.id, 'pxp'); // Doppel-Pass-XP-Aktion und Trank
        const pn = kpass.norm(u), before = Number(pn.pass_xp ?? u.pass_xp) || 0;
        Object.assign(out.user, pn, { pass_xp: before + gain });
        out.summary.pass = { gain, tierBefore: kpass.tierOf(before), tierAfter: kpass.tierOf(before + gain) };
      }
      ...
      // Nur die Änderungen dieses Matches speichern, und zwar auf den frischen Stand: Zahlen als Differenz,
      // damit XP, Diamanten usw., die inzwischen woanders dazukamen (Herausforderungen, Wetten, Casino), nicht überschrieben werden
      if (process.env.MATCH_SAVE_DELAY) await new Promise((r) => setTimeout(r, Number(process.env.MATCH_SAVE_DELAY))); // nur für Tests
      const fresh = (await store.userById(p.id)) || u, upd = {};
      for (const k of Object.keys(out.user)) {
        const a = out.user[k], b = u0[k]; if (a === b) continue;
        const na = Number(a), nb = Number(b ?? 0);
        if (typeof a !== 'object' && a !== '' && Number.isFinite(na) && Number.isFinite(nb) && !(typeof a === 'string' && typeof b === 'string' && isNaN(Number(b)))) upd[k] = (Number(fresh[k]) || 0) + (na - nb);
        else upd[k] = a;
      }
      if ('xp' in upd) upd.xp = Math.max(0, Math.min(progress.capFor(u.prestige), upd.xp));
      if (Number(fresh.prestige) !== Number(u0.prestige)) delete upd.xp; // in der Zwischenzeit Prestige gemacht: XP nicht anfassen
      await store.save(p.id, upd);
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('create', (opts) => { // Modus gleich beim Eröffnen: kein Umschalten danach, nichts flackert
      ...
      const m = newMatch();
      if (opts && opts.mode === 'ranked') { m.ranked = true; m.themes = [...themes.RANKED]; }
      else m.themes = themes.valid(['allgemein']); // freies Spiel und Solo: nur Allgemeinwissen vorausgewählt, weitere Themen wählt man selbst
    ...
    async function join(m) {
      ...
      m.players.set(user.id, { id: user.id, name: user.name, color: COLORS.find((c) => !used.has(c)), score: 0, gain: 0, answer: null, sockets: new Set([socket]),
        ...
        level: progress.levelInfo(u.xp || 0, u.prestige).level, prestige: u.prestige || 0, presShown: (u.pres_shown === -1 ? 0 : u.pres_shown === 12 && String(u.unlocks || '').split(',').includes('EX5') ? 12 : u.pres_shown > 0 ? Math.min(u.pres_shown, u.prestige || 0) : (u.prestige || 0)), av: u.av || 0, tag: u.tag || '', tagColor: u.tag_color || '', emblem: u.emblem || '', streak: u.streak || 0, frame: u.frame || '', frameAnim: frames.animOf(u.frame),
    ...
    socket.on('create_solo', () => {
      ...
      m.settings.pointLimit = 2000; m.settings.maxQuestions = m.settings.maxQuestions <= 20 ? 15 : 25; // Solo: fest bis 2.000 Punkte, 15 oder 25 Fragen
    ...
    socket.on('report', () => {
      ...
      m.reportedBy.add(user.id); store.report(m.current.id, user.name).catch(() => {}); push(m);
    });
    ...
    socket.on('again', () => {
      ...
      if (m.solo && m.players.size === 1) { // Solo: gleich die nächste Solo-Runde starten, nicht in eine offene Lobby
        m.settings.pointLimit = 2000; m.settings.maxQuestions = m.settings.maxQuestions <= 20 ? 15 : 25;
        startCountdown(m);
        if (!ai.enabled()) { m.readyGo = true; timer(m, 4000, () => nextQuestion(m)); }
        push(m); pushLobbies(); return;
      }
      push(m); pushLobbies();
    });
