...
module.exports = function attach(io, store) {
  ...
  async function prepareQuestions(m) {
    ...
    const unseen = shuffle(all.filter((q) => !m.asked.has(q.id))).sort((x, y) => bucket(x) - bucket(y));
    ...
    const known = await store.askedAll(400).catch(() => []);
    ...
    const run = async (n, have = []) => {
      ...
      try {
        ...
        const ask = Math.min(45, Math.ceil(need * 1.3) + 2);
        const r = await ai.generate(ask, [...m.questions.map((q) => q.q), ...lastAsked, ...known], { themes: themeList, timeout: m.waitAi ? 230000 : Math.max(15000, m.settings.countdown * 1000 - 4000) });
        if (r && r.length) have = [...have, ...(await fresh_ok(r.filter((q) => !have.some((x) => x.q === q.q))))];
