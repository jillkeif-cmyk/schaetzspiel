...
module.exports = function attach(io, store) {
  ...
  function activityOf(id) {
    ...
    if (hooks.table(id)) return 'sitzt am Blackjack-Tisch';
  ...
  function lobbies() {
    return {
      max: MAX_MATCHES, maxPlayers: MAX_PLAYERS, running: matches.size, online: online.size, onlineNames: [...online.values()].map((o) => o.name).slice(0, 12), onlinePeople: [...online.entries()].slice(0, 20).map(([id, o]) => ({ id, name: o.name, act: activityOf(id) })),
      open: [...matches.values()].filter((m) => !m.solo).map((m) => ({ code: m.code, host: m.players.get(m.hostId)?.name || '?', players: m.players.size, phase: m.phase, setup: m.setup, locked: !!m.password, ranked: m.ranked, qIndex: m.qIndex, qTotal: m.questions.length || m.settings.maxQuestions })),
    };
  }
  ...
  function stateFor(m, uid, watching = false) {
    ...
    return {
      code: m.code, phase: m.phase, hostId: m.hostId, you: uid, watching, watchers: m.watchers.size,
      ...
      aiStatus: m.aiStatus, startWhenReady: !!m.startWhenReady, readyGo: !!m.readyGo, qIndex: m.qIndex, qTotal: m.questions.length || m.settings.maxQuestions,
  ...
  async function prepareQuestions(m) {
    ...
    const fresh_n = m.waitAi ? m.questions.length : Math.ceil(m.questions.length * 0.4);
    const tries = m.waitAi ? 3 : 1;
    const known = history;
    ...
    const run = async (n, have = []) => {
      const need = fresh_n - have.length;
      if (need <= 0) return have;
      try {
        // etwas mehr anfordern, weil beim Prüfen immer welche wegfallen
        const ask = Math.min(45, Math.ceil(need * (m.waitAi ? 1.6 : 1.3)) + 2);
        const r = await ai.generate(ask, [...lastAsked.slice(-40).reverse(), ...known.slice(0, 80)], { themes: themeList, timeout: m.waitAi ? 230000 : Math.max(15000, m.settings.countdown * 1000 - 4000) });
        if (r && r.length) have = [...have, ...(await fresh_ok(r.filter((q) => !have.some((x) => sim.similar(x.q, q.q, x.a, q.a)))))];
        if (r && r.length) console.log(`KI-Prüfung: ${r.length} geliefert, ${have.length} von ${fresh_n} frisch und neu`);
      } catch (e) { console.error('KI-Versuch ' + n + ' fehlgeschlagen:', e.message); }
      if (have.length >= fresh_n || n >= tries) return have;
      await new Promise((r) => setTimeout(r, 1500)); // kurz warten, nicht sofort nachfeuern
      return run(n + 1, have);
    };
    run(1).then(async (raw) => {
      const fresh = raw.filter(themeMatch);
      const stored = await store.addAiQuestions(fresh).catch(() => fresh.map((q, i) => ({ ...q, id: 'x' + Date.now() + i })));
      if (m.phase !== 'countdown' || !stored.length) { m.aiStatus = stored.length ? 'ready' : 'failed'; if (matches.has(m.code)) { push(m); if (m.phase === 'countdown' && m.startWhenReady) timer(m, 1500, () => nextQuestion(m)); } return; }
      // Frische KI-Fragen ersetzen einen Teil der Pool-Fragen
      const n = m.questions.length;
      if (m.waitAi && stored.length >= Math.min(n, 5)) {
        m.questions = shuffle(stored).slice(0, n); // Warten auf frische Fragen: keine Pool-Fragen dazumischen, lieber etwas kürzer
      } else {
        const keep = m.questions.slice(0, Math.max(0, n - stored.length)).filter((q) => !stored.some((x) => sim.similar(x.q, q.q, x.a, q.a)));
        m.questions = shuffle([...keep, ...stored]).slice(0, n);
      }
      for (const q of m.questions) m.asked.add(q.id);
      const wasWaiting = m.aiStatus === 'waiting';
      m.aiStatus = 'ready'; if (wasWaiting) m.startWhenReady = true; push(m); readyGo(m);
    }).catch((e) => { console.error('KI-Fragen fehlgeschlagen:', e.message); m.aiStatus = 'failed'; if (matches.has(m.code)) { push(m); if (m.phase === 'countdown' && m.startWhenReady) timer(m, 1500, () => nextQuestion(m)); } });
  }
  ...
  function finish(m) {
    ...
    Promise.all(ps.filter((p) => p.match).map(async (p) => {
      ...
      if (solo && (r.answered || 0) > 0) {
        ...
        out.user.xp = Math.max(0, (out.user.xp || 0) - cut);
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('disconnect', () => {
      ...
      const o = online.get(user.id); if (o) { o.sockets.delete(socket); if (--o.n <= 0) { online.delete(user.id); where.delete(user.id); } } pushLobbies();
      store.save(user.id, { last_seen: Date.now() }).catch(() => {});
  ...
  return { matches, scoreEstimate, online, userMatch, activityOf, hooks };
};
