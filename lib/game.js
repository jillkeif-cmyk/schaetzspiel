...
module.exports = function attach(io, store) {
  ...
  function finish(m) {
    ...
    store.ctx.run({ src: 'Quiz-Match' }, () => Promise.all(ps.filter((p) => p.match).map(async (p) => {
      ...
      if ('xp' in upd) upd.xp = Math.max(0, Math.min(progress.CAP, upd.xp));
  ...
  io.on('connection', (socket) => {
    ...
    async function join(m) {
      ...
      m.players.set(user.id, { id: user.id, name: user.name, color: COLORS.find((c) => !used.has(c)), score: 0, gain: 0, answer: null, sockets: new Set([socket]),
        ...
        level: progress.levelInfo(u.xp || 0).level, prestige: u.prestige || 0, presShown: (u.pres_shown === -1 ? 0 : u.pres_shown === 12 && String(u.unlocks || '').split(',').includes('EX5') ? 12 : u.pres_shown > 0 ? Math.min(u.pres_shown, u.prestige || 0) : (u.prestige || 0)), av: u.av || 0, tag: u.tag || '', tagColor: u.tag_color || '', emblem: u.emblem || '', streak: u.streak || 0, frame: u.frame || '', frameAnim: frames.animOf(u.frame),
