...
const FS_START = 12;
let STRIPS = ["SSHHPXEKKKLLHHPKKKJJJXBBKKKQQQBBEAAAEJJJXAAALLQQQAAAHHLLSSWJJJQQQ", "KKKSSQQQLLXHHLLHHJJJAAAHHBBLLEBBAAAEPXKKKQQQWJJJAAAKKKQQQJJJXESSP", "AAAJJJSSBBQQQAAAKKKXEQQQHHXHHBBLLKKKPJJJQQQLLKKKAAAPSSLLEJJJHHXWE", "LLJJJBBKKKHHJJJEAAAEJJJQQQAAAKKKLLPAAAWSSHHQQQKKKELLBBXQQQXHHXSSP", "WHHAAAQQQBBXEPHHEHHXBBKKKLLAAALLSSKKKLLAAAKKKQQQJJJXEJJJPSSJJJQQQ"].map((x) => x.split('')); // Grundspiel: gestapelte Symbole (z. B. AAA) wie im Original
const up = (x, lvl) => { const i = TIERS.indexOf(x); return i >= 0 && i < lvl ? TIERS[lvl] : x; };
let FS_STRIPS = ["AAASSHHXJJJKKKEKKKBLLWKKKHHLLBQQQAAASSLLJJJSSPHHJJJAAAQQQBQQQ", "HHLLBQQQHHJJJXQQQAAAEWSSHHBQQQKKKJJJAAASSPKKKLLKKKAAAJJJBSSLL", "KKKQQQKKKPJJJLLHHJJJSSKKKQQQAAABAAAXLLWELLHHBSSAAABHHQQQJJJSS", "SSAAASSAAALLHHBJJJLLKKKQQQSSBJJJKKKPKKKELLHHXQQQHHAAAQQQJJJBW", "QQQAAAHHSSQQQLLWKKKQQQBHHAAAJJJLLKKKJJJSSBLLHHBAAASSXKKKJJJPE"].map((x) => x.split('')); // Freispiele: eigene Walzen, gestapelte Bildsymbole → nach Verbesserungen oft volle Bilder
const spinGrid = (rnd, lvl = 0, fs = false) => (fs && FS_STRIPS ? FS_STRIPS : STRIPS).map((s) => { const i = rnd(s.length); return [0, 1, 2].map((k) => up(s[(i + k) % s.length], lvl)); });
...
function play(bet, rnd = randomInt, force = false) { // force: Admin-Test, löst Freispiele aus
  ...
  if (base.sc >= 3) { // Freispiele: 12, jeder Anubis +1 und Tafel-Stufe nach der Auswertung
    ...
    while (left > 0 && n < 200) {
      left--; n++;
      const g = spinGrid(rnd, lvl, true), r = evaluate(g, bet); let ws = 0; g.forEach((col) => col.forEach((x) => { if (x === 'W') ws++; }));
...
function setStrips(s, fs) { STRIPS = s.map((x) => x.split('')); if (fs) FS_STRIPS = fs.map((x) => x.split('')); }
