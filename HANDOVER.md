...
- Kirmes - Runde 24.09.: Rakete (Update 19, Admin-Freigabe kirmes_live): lib/kirmes.js (Lukas 96 %, Rennen ~95 %, Greifer 95 %, Rakete 96 %), Münzschieber als GEMEINSAMER Automat (PM in server.js: ein Sitz, 90 s Ablösung, Pool in Setting pusher_state, 1 Münze = 100 💎, 1–100 pro Einwurf, 95 % in den Pool, Fall nach Füllstand, Quote auf Dauer 95 %, Zuschauen per Socket-Raum pusher). Grafiken public/kirmes/, Klänge kmSnd().
- Runde 24.09.: schmale Bildschirme (Grids minmax(0,1fr), Themen-Umbruch, 340px → 2 Spalten), Tagesbonus/Glücksrad wackeln (.u2-sb.wig), Kronenpass hüpft bei Abholbarem (passV claimable + Casino-Strang), Roulette dreht von letzter Stellung weiter (window.__rDeg) und Kessel größer (min(68vw,270px,100dvh−575px), Zahlenverlauf als Spalte links, ‚ⓘ Regeln‘), Poker: Chips mit Kantenstreifen, Einsätze näher am Spieler (lerp 0.34), Pot als Haufen, pk-own/pk-stackchips aus, Werbekarte #promoCard sichtbar + home:cfg aktualisiert Kopf sofort, Zurück-Knopf beim Sitzen aus.
- Match: Spieler-Chips wieder sichtbar (mobonly-Regel nur Startseite), Emotes verstreut mit Namen, neue Matches mit Standard-Regeln (DEFAULTS = PRESET). REWIN-Rad neu (public/tcg/rewin_rim|rewin_hub.webp) mit echter Logik: getroffenes grünes Feld wird rot (stopSegs). Zurück zur Startseite: .u2-home im Casino, #back in Karten/Shop/Freunde.
- Schnellstopp an allen Automaten: START während des Drehens hält alle Walzen sofort an (tc.stopNow in tcAnimate/jcAnimate, tcStartKey). Ergebnis unverändert, kommt vorher vom Server.
- Shop-Angebot „Sammler-Truhe“ (server.js OFFER, offerCfg in Setting offer_cfg: on + buyers; /api/shop/offer einmal pro Spieler, /api/admin/offer Käufer + an/aus; Bild public/news/offer_truhe.webp; Inhalt: 1 Gruselnacht-Display, 3 GN, 3 Toon, 5 Geister, 5 Premium, 5 Standard, je 2 XP-/Casino-/Pass-Trank ×4). Support-Knopf rechts (i_support). Match-Eröffnung mit Modus: socket 'create' {mode: free|ranked|solo}, freie/Solo-Matches starten nur mit Allgemeinwissen. Maschinenwahl kompakt (← Automaten in der Diamanten-Zeile). Kessel 214 px. Duell-Hinweis und Gewinn-Text im Shop entfernt.
