...
- Auge des Anubis (lib/anubis.js, Idee wie Merkurs Eye of Horus, Nacht-Ägypten-Grafik in public/an/): 5×3, 10 Linien, Anubis W dehnt sich auf die Walze aus (ersetzt alles außer Grabtor P). 3+ Grabtore zahlen 1×/10×/25× Einsatz und geben 12 Freispiele (fest, laut Original-Gewinntabelle). In Freispielen: jeder Anubis +1 Freispiel und NACH der Auswertung Tafel +1 Stufe (Lotus→Ankh→Skarabäus→Falke→Schakal→Auge). Freispiele werden serverseitig am Stück berechnet (r.fs.spins). Auszahlung ≈ 95,6 % (1,5 Mio.), Freispiele ≈ alle 149 Drehs, Ø 36×. Maschinen 5–6 (TC_MACHINES 6, machineGame), /api/casino/anubis, Testphase anubis_test (Standard AN), Admin-Test-Knopf „👁 Freispiele“. Oberfläche: anPlay/anSpin/anExpand/anBeam/anPopup, Tafel #anTab, Zähler #anFs, Klänge anspin/anstop/anexp/anfs/anextra/anbeam.
- Freigabe neue Automaten: Admin „🚀 Neue Automaten freigeben“ → /api/admin/slotsrelease: Testphasen aus, Setting slots_live = 1, Update 14 (u14-automaten, requires: 'slots', newsVisible) sichtbar, Code AUTOMATEN (15.000 💎, 30 Tage) angelegt. Admin-Test-Knöpfe: „👁 Freispiele“ (Anubis), „🃏 Kappe“, „👑 Vollbild“ (nur Admin).
...
- Auge des Anubis Tafel-Verbesserung: anBeam(lvlAfter, lvlBefore) dreht den ausgedehnten Anubis (.an-turn) und schießt einen Laser (.an-laser, aus echten Positionen berechnet) auf jeden umgewandelten Tafelplatz.
