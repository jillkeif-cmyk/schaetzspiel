...
- Start: statischer Startbildschirm (#bootSplash in #app, wird von render() ersetzt), nach 9 s Hinweis + „Neu laden“ (script#bootJs – Hauptskript bleibt der letzte <script> ohne Attribute). Deploy: autoDeploy bei Push ist an → KEIN zusätzlicher manueller Deploy (sonst Doppel-Neustart). Empfehlung: Health Check Path /api/version in Render setzen.
